...
public class SocketServer {
	...
	protected synchronized void disconnect(ServerThread client) {
		...
		broadcast("disconnected", id, client.getName());
	}

	// Broadcast given message to everyone connected
	public synchronized void broadcast(String message, long id, String username) {
		// let's temporarily use the thread id as the client identifier to
		// show in all client's chat. This isn't good practice since it's subject to
		// change as clients connect/disconnect
		message = String.format("%d: %s", username, message);
		// end temp identifier
		...
		while (it.hasNext()) {
			...
			if (!wasSuccessful) {
				...
				it.remove();
				broadcast("Disconnected", id, username);
			}
		}
	}
	...
	public static void main(String[] args) {
		...
		int port = 3333;// make some default
		if (args.length >= 1) {
			String arg = args[0];
			try {
